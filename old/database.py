...
class Database:
    ...
    def register_user(self, username, password, password2, usertype):
        ...
        # error message if password doesn't meet all strength requirements
            ...
            password = self.hash_string(password)
            register_statement = "INSERT INTO Users(Username, Password, Score, UserType) VALUES (?, ?, ?, ?);" 
    ...
    def upload_question(self, diff, type):
        ...
        question = q.Question
        question_matrix = q.MatrixQuestion
        small_hint = q.SmallHint
        big_hint = q.BigHint

        statement = "INSERT INTO Questions(UserID, QuestionType, QuestionDiff, Question, SmallHint, BigHint) VALUES(?, ?, ?, ?, ?, ?);"
        values = (self.UserID, type, diff, question, small_hint, big_hint)
        self.cursor.execute(statement, values)
        ...
        if matrix2 is None:
            statement = "INSERT INTO MatricesVectors(QuestionID, Matrix) VALUES(?, ?);"
            values = (questionID, matrix)
            self.cursor.execute(statement, values)
        else:
            statement = "INSERT INTO MatricesVectors(QuestionID, Matrix) VALUES(?, ?);"
            values = (questionID, matrix)
